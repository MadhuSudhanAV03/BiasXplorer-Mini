...
export default function Dashboard() {
  ...
  const workingFilePath = useMemo(() => selectedFilePath || filePath, [selectedFilePath, filePath]);
  ...
  useEffect(() => {
    ...
    const problematic = entries.find(([, v]) => ["Moderate", "Severe"].includes(v?.severity));
      ...
      <div className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100">
        <div className="max-w-5xl mx-auto px-4 py-10">
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-6 text-amber-900">
            ...
            <p className="text-sm mb-4">Go back to the Home page and upload a dataset to begin.</p>
            <Link className="inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700" to="/">Go to Home</Link>
          </div>
        </div>
      </div>
    );
    ...
    <header className="border-b border-slate-200 bg-white/70 backdrop-blur sticky top-0 z-10">
      <div className="max-w-6xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-extrabold text-blue-600 tracking-tight">Dashboard</h1>
          <div className="text-xs text-slate-600">File: <span className="font-mono">{workingFilePath || "(none)"}</span></div>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 grid grid-cols-6 text-[11px] text-slate-600">
            {STEPS.map((label, i) => {
              ...
              return (
                <div key={label} className={`flex items-center gap-2 ${active ? "text-blue-700" : done ? "text-slate-800" : "text-slate-500"}`}>
                  <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-semibold border ${done ? "bg-blue-600 text-white border-blue-600" : active ? "border-blue-600 text-blue-600" : "border-slate-300"}`}>{stepNum}</span>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {currentStep === 2 && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            />
            <NavButtons onPrev={() => setCurrentStep(1)} onNext={() => setCurrentStep(3)} nextDisabled={!columns.length} />
          </section>
        )}
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <FeatureSelector
              ...
              onSelect={({ features, response }) => {
                if (response?.selected_file_path) setSelectedFilePath(response.selected_file_path);
                setCurrentStep(4);
              }}
            />
            <NavButtons onPrev={() => setCurrentStep(2)} onNext={() => setCurrentStep(4)} nextDisabled={!columns.length} />
          </section>
        )}
        ...
        {currentStep === 4 && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            />
            <NavButtons onPrev={() => setCurrentStep(3)} onNext={() => setCurrentStep(5)} nextDisabled={!categorical.length} />
          </section>
        )}
        ...
        {currentStep === 5 && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold">Choose Target Column</h2>
                <p className="text-xs text-slate-600">Pick a categorical column to apply correction.</p>
              </div>
        ...
        {currentStep === 6 && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <Visualization
              ...
              targetColumn={targetColumn}
            />
            <NavButtons onPrev={() => setCurrentStep(5)} onNext={() => setCurrentStep(6)} nextDisabled />
          </section>
        )}
      </main>
    </div>
